...
1. **SKILL.md** — YAML 프론트매터에 `name`과 `description`만 포함하고 Markdown 문서로 기술하는 Anthropic 표준 스킬 디스크립터
2. **엔트리 포인트** — 스킬 로직을 실행하는 스크립트(`skill.py`, `skill.js`) 또는 네이티브 바이너리
3. **manifest.json** — (선택) 레거시 JSON 스키마 폴백
...
→ 각 스킬 디렉토리의 SKILL.md (또는 manifest.json 폴백) 읽기
→ LLM 에이전트에 도구 스키마 등록
→ LLM이 런타임 실행기를 통해 호출
```
...
lib/
└── my_new_skill/
    ├── SKILL.md          # Anthropic 표준 디스크립터
    └── skill.py         # 또는 skill.js, main.cc
```

### 2. SKILL.md 작성 (Anthropic 표준)

`SKILL.md`는 LLM이 스킬을 탐색하고 이해하는 방식을 정의합니다. Anthropic 표준 형식을 사용합니다 — YAML 프론트매터에는 `name`과 `description`만, 본문은 Markdown 문서로 기술합니다:
...
이 스킬이 하는 일과 반환하는 데이터에 대한 명확한 설명.

## 사용 시점

사용자가 다음에 대해 질문할 때 사용합니다...

## 파라미터

| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| `example_param` | string | 아니오 | 파라미터에 대한 설명 |

## 출력

| 필드 | 타입 | 설명 |
|------|------|------|
| `key` | string | 출력 필드에 대한 설명 |
```
...
네이티브 스킬의 경우, `SKILL.md`에 바이너리 컨텍스트를 명시합니다:
...
`SKILL.md`는 LLM이 스킬을 이해하기 위해 읽는 문서입니다. 설명은 다음과 같이 작성하세요:

- **구체적으로** — LLM에게 반환되는 데이터를 정확히 알려주기
- **행동 지향적으로** — 동사로 시작 (예: "Get", "Query", "Check")
- **문맥이 풍부하게** — "사용 시점" 섹션에 예시 시나리오 포함
- **파라미터 완전하게** — 모든 파라미터를 표 형태로 명확한 설명과 타입 기술
