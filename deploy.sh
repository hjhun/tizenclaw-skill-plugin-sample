#!/bin/bash
# TizenClaw Skill Plugin Sample Build, Deploy & Run Script
# Automates: gbs build → sdb push → rpm install → unified-backend (app register)
#
# Usage:
...
PKG_NAME="tizenclaw-skill-plugin-sample"
APP_ID="org.tizen.tizenclaw-skill-plugin-sample"
...
usage() {
  cat <<EOF
${BOLD}TizenClaw Skill Plugin Sample Build & Deploy${NC}
...
do_deploy() {
  ...
  for rpm in "${RPM_FILES[@]}"; do
    ...
    run sdb_shell unified-backend --preload -y "${APP_ID}"
    ok "Plugin registered to registry"
  done

  log "Restarting tizenclaw daemon..."
...
show_summary() {
  ...
  echo ""
  log "The TizenClaw SkillPluginManager will auto-detect and register the skills."
  log "To verify it loaded successfully, use the following command:"
  log "  sdb shell dlogutil TIZENCLAW"
  echo ""
}
